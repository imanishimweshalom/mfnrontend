...
export default function StoryPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const approvedComments = allComments.filter((comment) => {
          const status = String(
            comment.status ||
        ...
        setPopular(
          Array.isArray(pRes.data)
            ? pRes.data
            : pRes.data?.stories || pRes.data?.data || []
        );
  ...
  useEffect(() => {
    ...
    if (showCommentModal || showShareMenu) {
  ...
  // ---------------------------------------------------------
  const refreshApprovedComments = async () => {
  ...
  const handleComment = async (e) => {
    ...
    try {
      ...
      // Refresh, but only approved comments will be displayed
  ...
  // ---------------------------------------------------------
  // NATIVE SHARE
  // Useful for Instagram and other apps supported by device
  // ---------------------------------------------------------
      ...
      <div className="mhk-layout-grid">
        ...
        <article className="mhk-center-col">
          ...
          >
            ...
            <span>
              {timeAgo(story.created_at || story.createdAt)}
            </span>
...
/* ============================================================
   ROBUST ADCARD
   Supports IMAGE + VIDEO advertisements
============================================================ */
