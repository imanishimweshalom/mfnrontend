import React, { useState, useEffect, useMemo, useRef } from 'react';import { useParams, Link } from 'react-router-dom';import PublicLayout from '../../components/layout/PublicLayout';import { PopularItem, AdBanner, SectionLabel, NewsletterWidget, WhatsAppCTA, Spinner, EmptyState, imgUrl, timeAgo } from '../../components/ui';import { storiesAPI, commentsAPI, adsAPI } from '../../utils/api';

// Fix: Embedded Data URI placeholder. This guarantees an image will always show,// even if the backend placeholder file is missing or fails to load.const PLACEHOLDER = "data/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' width='16' height='16'%3E%3Crect width='16' height='16' fill='%23e8e4d8'/%3E%3C/svg%3E";

export default function StoryPage() {const { id } = useParams();const [story, setStory] = useState(null);const [related, setRelated] = useState([]);const [comments, setComments] = useState([]);const [popular, setPopular] = useState([]);const [ads, setAds] = useState([]);const [loading, setLoading] = useState(true);const [form, setForm] = useState({ name: '', email: '', comment: '' });const [submitting, setSubmitting] = useState(false);const [commentMsg, setCommentMsg] = useState('');const [reactions, setReactions] = useState({ likes: 0, dislikes: 0 });

const [showCommentModal, setShowCommentModal] = useState(false);const [refreshingComments, setRefreshingComments] = useState(false);

// Simplified state for the bottom carouselconst [adIndex, setAdIndex] = useState(0);

useEffect(() => {window.scrollTo(0, 0);const load = async () => {setLoading(true);try {const [sRes, cRes, pRes, aRes] = await Promise.all([storiesAPI.getOne(id),commentsAPI.getByStory(id),storiesAPI.getPopular({ limit: 5 }),adsAPI.getAll(),]);const s = sRes.data;setStory(s);setReactions({ likes: s.likes || 0, dislikes: s.dislikes || 0 });setComments(cRes.data || []);setPopular(pRes.data || []);
...
// Restrict ad areas to exactly 3 ads per sectionconst leftAds = useMemo(() => activeAds.slice(0, 3), [activeAds]);const floatAds = useMemo(() => activeAds.slice(0, 3), [activeAds]);

// Simple interval to cycle the bottom ad carousel every 8 secondsuseEffect(() => {if (floatAds.length === 0) return;const timer = setInterval(() => {setAdIndex(prev => (prev + 1) % floatAds.length);}, 8000);return () => clearInterval(timer);}, [floatAds]);

useEffect(() => {document.body.style.overflow = showCommentModal ? 'hidden' : '';return () => { document.body.style.overflow = ''; };}, [showCommentModal]);

useEffect(() => {const onKey = (e) => { if (e.key === 'Escape') setShowCommentModal(false); };if (showCommentModal) window.addEventListener('keydown', onKey);return () => window.removeEventListener('keydown', onKey);}, [showCommentModal]);

const handleReact = async (type) => {try {const res = await storiesAPI.react(id, type);setReactions(res.data);} catch (e) { console.error(e); }};

const handleComment = async (e) => {e.preventDefault();setSubmitting(true);setCommentMsg('');try {await commentsAPI.create({ story_id: id, ...form });setCommentMsg('✅ Comment submitted for review!');setForm({ name: '', email: '', comment: '' });setRefreshingComments(true);try {const cRes = await commentsAPI.getByStory(id);setComments(cRes.data || []);} catch (_) {}finally { setRefreshingComments(false); }setTimeout(() => setCommentMsg(''), 4000);} catch { setCommentMsg('❌ Error submitting. Please try again.'); }finally { setSubmitting(false); }};

if (loading) return <PublicLayout><Spinner /></PublicLayout>;if (!story) return <PublicLayout><EmptyState icon="📰" title="Story not found" message="This story may have been removed." /></PublicLayout>;

const authorAvatar = story.author_avatar || story.author_image;

return (<PublicLayout><style>{`@keyframes mhkFade { from { opacity: 0; } to { opacity: 1; } }@keyframes mhkPop { from { transform: translateY(40px) scale(.98); opacity: 0; } to { transform: translateY(0) scale(1); opacity: 1; } }@keyframes mhkSpin { to { transform: rotate(360deg); } }@keyframes mhkAdFade { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
...
// Check all common API property names for the media fileconst rawMedia = ad.image_url || ad.imageUrl || ad.image || ad.banner || ad.bannerUrl || ad.img || ad.file_url || ad.photo || ad.file;const media = rawMedia ? imgUrl(rawMedia) : null;

// Determine if the ad is a video based on type or file extensionconst isVideo = ad.type === 'video' || (rawMedia && /.(mp4|webm|ogg|mov)$/i.test(rawMedia));

const link = ad.link || ad.url || ad.click_url || ad.target_url || '#';const title = ad.title || ad.name || 'Sponsored Ad';

return (<ahref={link}target="_blank"rel="noopener noreferrer sponsored"className={mhk-ad-card ${className}}style={{width: '100%',height: fluid ? '100%' : height,textDecoration: 'none',background: '#fff',marginBottom: fluid ? 0 : 12,flex: fluid ? 1 : 'unset'}}>{media ? (isVideo ? (<videosrc={media}autoPlaymutedloopplaysInlinestyle={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', flex: 1 }}/>) : (<imgsrc={media}alt={title}loading="lazy"onError={e => { e.target.onerror = null; e.target.src = PLACEHOLDER; }}style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', flex: 1 }}/>)) : (<div style={{width: '100%', height: '100%',display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',color: '#bbb', fontFamily: "'Barlow Condensed',sans-serif", fontSize: 14,flex: 1, background: '#f8f9fa', padding: '10px', textAlign: 'center'}}><span style={{ fontWeight: 700, marginBottom: 4 }}>{title}</span><small style={{ fontSize: 10, opacity: 0.7 }}>Ad Space Available</small></div>)}<div style={{ padding: '4px 10px', fontFamily: "'Barlow Condensed',sans-serif", fontSize: 8, letterSpacing: 1.5, textTransform: 'uppercase', color: '#bbb', background: '#f0ece0' }}>Advertisement</div></a>);});
